...
get_taxon_matrix <-
  function(long_dt = long_table_dt,
           group_var = "RESERVE.GROUP.LOCATION",
           level = "GENUS") {
    if (level == "GENUS") {
      # aggregate dt for provided grouping variable
      long_table_dt_agg_group_var_level <-
        long_dt[, lapply(.SD, sum, na.rm = TRUE), by = c(group_var,
                                                         "SUPERKINGDOM",
                                                         "PHYLUM",
                                                         "CLASS",
                                                         "ORDER",
                                                         "FAMILY",
                                                         "GENUS"), .SDcols = c("BOTH.PRES")]
    } else if (level == "SPECIES") {
      # aggregate dt for provided grouping variable
      
      long_table_dt_agg_group_var_level <-
        long_dt[, lapply(.SD, sum, na.rm = TRUE), by = c(
          group_var,
          "SUPERKINGDOM",
          "PHYLUM",
          "CLASS",
    ...
    taxon_matrix <-
      as.matrix(
        data.table::dcast(
          setDT(long_table_dt_agg_group_var_level),
          get(group_var) ~ get(level),
          value.var = "BOTH.PRES",
          sum,
          fill = 0
        ),
...
# ================
long_table <-
  readRDS(file = "/Users/paul/Documents/OU_eDNA/200403_manuscript/5_online_repository/R_objects/210301_997_r_format_longtables__analysis_input.Rds")
mdl_specs <-
  read_csv(
...
long_table <-
  long_table %>% mutate(BRUV.PRES = case_when(SAMPLE.TYPE == "BRUV" &
                                                ABUNDANCE >= 1 ~ 1, TRUE ~ 0))
long_table <-
  long_table %>% mutate(EDNA.PRES = case_when(SAMPLE.TYPE == "eDNA" &
                                                ABUNDANCE >= 1 ~ 1, TRUE ~ 0))
long_table <-
  long_table %>% mutate(BOTH.PRES = case_when(BRUV.PRES == 1 |
                                                EDNA.PRES == 1 ~ 1, TRUE ~ 0))
...
long_table <-
  long_table %>% mutate(GENUS = case_when(
    GENUS %in% nonnz_fish ~ paste0(GENUS, "*"),
...
# https://gis.stackexchange.com/questions/243569/simplify-polygons-of-sf-object
nzshp_lores_WGS84 <-
  rmapshaper::ms_simplify(input = as(nzshp_hires_WGS84, 'Spatial')) %>% st_as_sf()
...
bbox <- long_table_dt_map %>%
  ...
  summarise(
    xmin = min(MH.PPS.LONG) - 0.01 ,
    ymin = min(MH.GPS.LAT) - 0.01,
    xmax = max(MH.PPS.LONG) + 0.01,
    ymax = max(MH.GPS.LAT) + 0.01
  ) %>%
...
map_main <- ggplot(data = nzshp_lores_WGS84) +
  ...
  coord_sf(xlim = c((166.5 - 0.1), (167.0 + 0.1)),
           ylim = c((-46.04 - 0.1), (-45.52 + 0.1)),
           expand = FALSE) +
  geom_point(
    data = long_table_dt_map,
    aes(x = MH.PPS.LONG, y = MH.GPS.LAT, shape = RESERVE.GROUP),
    color = "darkred",
    size = 4
  ) +
  geom_point(
  ...
  geom_sf_label(
    data = bbox,
    aes(label = RESERVE.GROUP.LOCATION),
    nudge_x = 0.06,
    nudge_y = 0.06
  ) +
  theme_bw() +
...
ggsave(
  "210401_998_r_summarize_results_fig1_map-BRUVonly.pdf",
  plot = last_plot(),
  device = "pdf",
...
ggsave(
  "210407_998_r_summarize_results_observations-BRUVonly.pdf",
  plot = last_plot(),
  device = "pdf",
  path = "/Users/paul/Documents/OU_eDNA/200403_manuscript/7_si_auxillary_files",
  scale = 1,
...
ggsave(
  "210407_998_r_summarize_results_observations_heat_BRUVonly.pdf",
  plot = last_plot(),
  device = "pdf",
  path = "/Users/paul/Documents/OU_eDNA/200403_manuscript/7_si_auxillary_files",
  scale = 1,
...
ggsave(
  "210312_998_r_summarize_results_jaccard_BRUVonly.pdf",
  plot = last_plot(),
  device = "pdf",
  path = "/Users/paul/Documents/OU_eDNA/200403_manuscript/7_si_auxillary_files",
  scale = 1,
...
long_table_dt_sfct <-
  long_table_dt[, c("RESERVE.GROUP.INSIDE", "RESERVE.GROUP.LOCATION", "GENUS")] %>% unique()
...
cats <-
  apply(long_table_dt_sfct, 2, function(x)
    nlevels(as.factor(x)))
cats
...
ggsave(
  "210312_998_r_summarize_results_mca_dim1_BRUVonly.pdf",
  plot = last_plot(),
  device = "pdf",
  path = "/Users/paul/Documents/OU_eDNA/200403_manuscript/7_si_auxillary_files",
  scale = 1.3,
...
ggsave(
  "210408_998_r_summarize_results_mca_BRUVonly.pdf",
  plot = last_plot(),
  device = "pdf",
  path = "/Users/paul/Documents/OU_eDNA/200403_manuscript/7_si_auxillary_files",
  scale = 1,
